...
import com.marotech.vending.api.RegisterRequest;
...
import com.marotech.vending.util.EmailValidator;
...
public class RegistrationController extends BaseController{
    ...
    register(@RequestBody RegisterRequest request) {
        ...
        try {
            ...
            if(request.getRegType() != null && request.getRegType() == RegType.AGENT
                    && StringUtils.isEmpty(request.getVendorId())){
                builder.append("Vendor must be specified if registering as an agent");
                createActivityLog(request.getMobileNumber(), null, request.getMobileNumber() + " tried to register as an agent but did not specify vendor via web services");
                LOG.error("Error: Vendor must be specified if registering as an agent: " + request);
            }
    ...
    private void createTheUser(RegisterRequest request, AuthUser authUser) {
        ...
        if (request.getRegType() == RegType.AGENT) {
            role = repositoryService.fetchUserRoleByRoleName("Agent");
            user.getUserRoles().add(role);
        }
        ...
        account.setAccountType(AccountType.TAX_AUTHORITY);
        account.setAccountStatus(AccountStatus.ACTIVE);
        account.setAccountName(user.getFullName());
        account.setAccountNumber(AccountNumberGenerator.generateTier1CardNumber());
        balance.setCurrency(config.getProperty(Constants.DEFAULT_CURRENCY));
        account.setAvailableBalance(balance);
        repositoryService.save(account);
        user.setAccount(account);
        repositoryService.save(user);

        if (request.getRegType() == RegType.AGENT) {
            Vendor vendor = repositoryService.fetchVendorById(request.getVendorId());
            vendor.getAgents().add(user);
            repositoryService.save(vendor);
            user.setVendor(vendor);
            user.setAgentStatus(AgentStatus.NOT_ACTIVE);
