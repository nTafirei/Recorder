...
import com.marotech.vending.model.AppSession;
import com.marotech.vending.model.AuthUser;
import com.marotech.vending.model.User;
import com.marotech.vending.model.Voucher;
...
public class VouchersController extends BaseController{
    ...
    @ApiOperation(value = "Fetches account vouchers from system",
            notes = "Fetches account vouchers from system")
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Token is valid. Voucher list is included"),
    ...
    getVouchers(@RequestBody VouchersRequest request) {
        ...
        try {
            ...
            List<VoucherDTO> transactions = new ArrayList<>();

            if(request.getStartDate() == null) {
                List<Voucher> vouchers = repositoryService.fetchVoucherTransactionsForUser(user, request.getPage());
                if (vouchers.isEmpty()) {
                ...
                for (Voucher voucher : vouchers) {
                    populateVoucherDTO(transactions, voucher);
                }
            }else{
                List<Voucher> vouchers = repositoryService.fetchVoucherTransactionsForUser(user, request.getStartDate(),
            ...
            response.getAdditionalInfo().put(VOUCHERS, GSON.toJson(transactions));
    ...
    private static void populateVoucherDTO(List<VoucherDTO> transactions, Voucher voucher) {
        VoucherDTO dto = new VoucherDTO();
        dto.setAmount(voucher.getOriginalAmount());
        ...
        dto.setProduct(voucher.getProduct().getName());
        dto.setMerchant(voucher.getProduct().getMerchant().getName());
        dto.setBeneficiaryMobileNumber(voucher.getBeneficiaryMobileNumber());
        dto.setBuyerMobileNumber(voucher.getBuyerMobileNumber());
        transactions.add(dto);
    }

    private void validateRequest(VouchersRequest request, ServiceResponse serviceResponse) {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(VouchersController.class);
}
