...
<s:layout-render name="/WEB-INF/jsp/layout.jsp" title="Agent Transaction History">
    ...
    <s:layout-component name="contents">
     <security:protected-element name="view-agent-transactions">
                ...
                <div id="content">
                    ...
                    <s:errors/>
                        <c:if test="${empty actionBean.transactions}">
                            <strong><fmt:message key="noagenttransactionsfound"/> for ${actionBean.agent.fullName}</strong>

                        </c:if>
                        <s:form action="/web/agent-transaction-history/${actionBean.agent.id}" name="createForm" id="createForm" method="post">
                            <table width="100%">
                                <tr>
                                    <td align="center" colspan = "3">
                                        <c:if test="${!empty actionBean.transactions}">
                                            ...
                                            ${actionBean.page.totalItemsFound} <fmt:message key="agenttransactionsfound"/>
                                            for ${actionBean.agent.fullName} of ${actionBean.agent.vendor.name}</strong>
                                                <br/>
                                                <c:forEach items="${actionBean.page.pageNumbers}" var="pageItem"
                                                                                          varStatus="loopStatus">
                                                  <d:link href="/web/agent-transaction-history/${actionBean.user.id}?currPage=${pageItem}">${pageItem}</d:link> |
                                                </c:forEach>
                                        </c:if>
                                    </td>
                                        <td>
                            ...
                            <c:if test="${!empty actionBean.transactions}">
                                ...
                                <c:forEach items="${actionBean.transactions}" var="transaction"
                                    ...
                                    <tr class="${loopStatus.index % 2 == 0 ? 'even' : 'odd'}">
                                        <td>
                                            ${transaction.formattedDayCreated}
                                        </td>
                                        <td>
                                    ...
                                    <tr>
                                        <td colspan="7" align="right">
                                                    <d:link target="_blank" href="/web/agent-transaction-history?exportType=PDF&_eventName=export&currPage=${actionBean.page.currPage}&startDate=${actionBean.startDate}&endDate=${actionBean.endDate}&agent=${actionBean.agent.id}">
                                                        <fmt:message key="exportropdflabel"/>
                                                    </d:link>
                                                    | <d:link target="_blank" href="/web/agent-transaction-history?exportType=EXCEL&_eventName=export&currPage=${actionBean.page.currPage}&startDate=${actionBean.startDate}&endDate=${actionBean.endDate}&agent=${actionBean.agent.id}">
                                                        <fmt:message key="exporttoexcellabel"/>
                                                    </d:link>
                                                    | <d:link target="_blank" href="/web/agent-transaction-history?exportType=CSV&_eventName=export&currPage=${actionBean.page.currPage}&startDate=${actionBean.startDate}&endDate=${actionBean.endDate}&agent=${actionBean.agent.id}">
