...
<s:layout-render name="/WEB-INF/jsp/layout.jsp" title="Merchant Transaction History">
                ...
                <div id="content">
                    ...
                    <s:errors/>
                        <c:if test="${empty actionBean.transactions}">
                            <strong><fmt:message key="nomerchanttransactionsfound"/>
                            for ${actionBean.merchant.name}.
                            <fmt:message key="balance"/>:  ${actionBean.merchant.account.availableBalance.amount}
                            </strong>
                        </c:if>
                        <s:form action="/web/merchant-transaction-history/${actionBean.merchant.id}" name="createForm" id="createForm" method="post">
                            <table width="100%">
                                <tr>
                                    <td align="center" colspan = "3">
                                        <c:if test="${!empty actionBean.transactions}">
                                            ...
                                            ${actionBean.page.totalItemsFound}<fmt:message key="merchanttransactions"/> for ${actionBean.merchant.name},
                                            page is ${actionBean.page.currPage}.

                                                    <fmt:message key="balance"/>:  ${actionBean.merchant.account.availableBalance.amount}</strong>
                                                    <br/>
                                                    <c:forEach items="${actionBean.page.pageNumbers}" var="pageItem"
                                                                                              varStatus="loopStatus">
                                                      <d:link href="/web/merchant-transaction-history/${actionBean.merchant.id}?currPage=${pageItem}">${pageItem}</d:link> |
                                                    </c:forEach>
                                        </c:if>
                                    </td>
                                        <td>
                                ...
                                <c:forEach items="${actionBean.transactions}" var="transaction"
                                    ...
                                    <tr>
                                        <td colspan="7" align="right">
                                                    <d:link target="_blank" href="/web/merchant-transaction-history?exportType=PDF&_eventName=export&currPage=${actionBean.page.currPage}&startDate=${actionBean.startDate}&endDate=${actionBean.endDate}&merchant=${actionBean.merchant.id}">
                                                        <fmt:message key="exportropdflabel"/>
                                                    </d:link>
                                                    | <d:link target="_blank" href="/web/merchant-transaction-history?exportType=EXCEL&_eventName=export&currPage=${actionBean.page.currPage}&startDate=${actionBean.startDate}&endDate=${actionBean.endDate}&merchant=${actionBean.merchant.id}">
                                                        <fmt:message key="exporttoexcellabel"/>
                                                    </d:link>
                                                    | <d:link target="_blank" href="/web/merchant-transaction-history?exportType=CSV&_eventName=export&currPage=${actionBean.page.currPage}&startDate=${actionBean.startDate}&endDate=${actionBean.endDate}&merchant=${actionBean.merchant.id}">
