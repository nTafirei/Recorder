...
<s:layout-render name="/WEB-INF/jsp/layout.jsp" title="User Transaction History">
            ...
            <div class="container">
                <div id="content">
                        ...
                        <c:if test="${empty actionBean.transactions}">
                            <strong><fmt:message key="nopersonaltransactionsfound"/> for ${actionBean.user.fullName}.
                            <fmt:message key="balance"/>: ${actionBean.account.availableBalance.currency} ${actionBean.account.availableBalance.amount}
                            </strong>
                        </c:if>
                        <s:form action="/web/my-transaction-history/${actionBean.currentUser.id}" name="createForm" id="createForm" method="post">
                            <table width="100%">
                                <tr>
                                    <td align="center" colspan = "3">
                                        <c:if test="${!empty actionBean.transactions}">
                                            ...
                                            ${actionBean.page.totalItemsFound} <fmt:message key="personaltransactionsfound"/> for ${actionBean.user.fullName}.
                                            <fmt:message key="balance"/>: ${actionBean.account.availableBalance.currency} ${actionBean.account.availableBalance.amount}</strong>
                                                <br/>
                                                <c:forEach items="${actionBean.page.pageNumbers}" var="pageItem"
                                                                                          varStatus="loopStatus">
                                                  <d:link href="/web/my-transaction-history/${actionBean.user.id}?currPage=${pageItem}">${pageItem}</d:link> |
                                                </c:forEach>
                                        </c:if>
                                    </td>
                                        <td>
                                ...
                                <c:forEach items="${actionBean.transactions}" var="transaction"
                                    ...
                                    <tr>
                                        <td colspan="7" align="right">
                                                    <d:link target="_blank" href="/web/my-transaction-history?exportType=PDF&_eventName=export&currPage=${actionBean.page.currPage}&startDate=${actionBean.startDate}&endDate=${actionBean.endDate}">
                                                        <fmt:message key="exportropdflabel"/>
                                                    </d:link>
                                                    | <d:link target="_blank" href="/web/my-transaction-history?exportType=EXCEL&_eventName=export&currPage=${actionBean.page.currPage}&startDate=${actionBean.startDate}&endDate=${actionBean.endDate}">
                                                        <fmt:message key="exporttoexcellabel"/>
                                                    </d:link>
                                                    | <d:link target="_blank" href="/web/my-transaction-history?exportType=CSV&_eventName=export&currPage=${actionBean.page.currPage}&startDate=${actionBean.startDate}&endDate=${actionBean.endDate}">
