...
<s:layout-render name="/WEB-INF/jsp/layout.jsp" title="Vendor Transaction History">
                ...
                <div id="content">
                    ...
                    <s:errors/>
                        <c:if test="${empty actionBean.transactions}">
                            <strong><fmt:message key="novendortransactionsfound"/> for ${actionBean.vendor.name}.
                            <fmt:message key="balance"/>: ${actionBean.vendor.account.availableBalance.amount}
                            </strong>
                        </c:if>
                        <s:form action="/web/vendor-transaction-history/${actionBean.vendor.id}" name="createForm" id="createForm" method="post">
                            <table width="100%">
                                <tr>
                                    <td align="center" colspan = "3">
                                        <c:if test="${!empty actionBean.transactions}">
                                            ...
                                            ${actionBean.page.numItemsShowing}
                                            of ${actionBean.page.totalItemsFound}.
                                            ${actionBean.page.totalItemsFound} <fmt:message key="vendortransactionsfound"/>
                                            for ${actionBean.vendor.name}.
                                            page is ${actionBean.page.currPage}

                                            <fmt:message key="balance"/>:${actionBean.vendor.account.availableBalance.amount}</strong>
                                        </c:if>
                                                  <br/>
                                                  <c:forEach items="${actionBean.page.pageNumbers}" var="pageItem"
                                                                                            varStatus="loopStatus">
                                                    <d:link href="/web/vendor-transaction-history/${actionBean.vendor.id}?currPage=${pageItem}">${pageItem}</d:link> |
                                                  </c:forEach>
                                    </td>
                                            <td>
                                ...
                                <c:forEach items="${actionBean.transactions}" var="transaction"
                                    ...
                                    <tr>
                                        <td colspan="7" align="right">
                                                    <d:link target="_blank" href="/web/vendor-transaction-history?exportType=PDF&_eventName=export&currPage=${actionBean.page.currPage}&startDate=${actionBean.startDate}&endDate=${actionBean.endDate}&vendor=${actionBean.vendor.id}">
                                                        <fmt:message key="exportropdflabel"/>
                                                    </d:link>
                                                    | <d:link target="_blank" href="/web/vendor-transaction-history?exportType=EXCEL&_eventName=export&currPage=${actionBean.page.currPage}&startDate=${actionBean.startDate}&endDate=${actionBean.endDate}&vendor=${actionBean.vendor.id}">
                                                        <fmt:message key="exporttoexcellabel"/>
                                                    </d:link>
                                                    | <d:link target="_blank" href="/web/vendor-transaction-history?exportType=CSV&_eventName=export&currPage=${actionBean.page.currPage}&startDate=${actionBean.startDate}&endDate=${actionBean.endDate}&vendor=${actionBean.vendor.id}">
