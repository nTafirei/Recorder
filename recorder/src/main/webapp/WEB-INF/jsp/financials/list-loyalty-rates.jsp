...
<s:layout-render name="/WEB-INF/jsp/layout.jsp" title="View Loyalty Rates">
        ...
        <div id="main-wrapper main-contents">
            <div class="container">
                <div id="content">
                    ...
                    <s:errors/>
                        <c:if test="${empty actionBean.loyaltyRates}">
                            <strong><fmt:message key="noloyaltyratesfound"/> for ${actionBean.searchDates}</strong>
                        </c:if>
                        ...
                        <table width="100%">
                            <tr>
                                <td align="center" colspan = "3">
                                    <c:if test="${!empty actionBean.loyaltyRates}">
                                        <strong>${actionBean.loyaltyRatesSize}
                                        <fmt:message key="loyaltyratesfound"/> for ${actionBean.searchDates}</strong>
                                    </c:if>
                                </td>
                             </tr>
                                 <tr>
                       ...
                       <s:form action="/web/loyalty-rates" name="createForm" id="createForm" method="post">
                        ...
                        <table class="alternating">
                            <c:if test="${!empty actionBean.loyaltyRates}">
                                ...
                                <tbody>
                                <c:forEach items="${actionBean.loyaltyRates}" var="loyaltyRate"
                                    ...
                                    <td>
                                        ${loyaltyRate.valuePerUnit}
                                    </td>
                                    <td>
                                        ${loyaltyRate.threshold}
                                    </td>
                                    <td>
                                        ${loyaltyRate.currency}
                                    </td>
                                        <td>
                                            ${loyaltyRate.date}
                                        </td>
                                        <td align="right">
                                        <c:if test="${loyaltyRate.canBeEdited}">
                                            <security:protected-element name="manage-loyalty-rates">
                                                         <d:link
                                                                     href="/web/edit-loyalty-rate?loyaltyRate=${loyaltyRate.id}">
